...
export function formatRisk(r: RiskResponse): string {
  ...
  return [
    ...
    `${escMd(posBar)}`,
    '',
    line('Total exposure',  `$${escMd(Number(r.total_exposure).toFixed(2))}`),
    line('Max bet',         `$${escMd(Number(r.max_bet_usd).toFixed(2))}`),
    line('Min edge',        `${escMd(Number(r.min_edge_percent).toFixed(1))}%`),
    line('Drawdown',        `${escMd(Number(r.current_drawdown_pct).toFixed(1))}%`),
    '',
